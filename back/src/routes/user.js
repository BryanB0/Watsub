...
import session from "express-session"; // Importation de express-session
...
router.use(
  session({
    ...
    cookie: { secure: process.env.NODE_ENV === "production" }, // Utiliser true en production avec HTTPS
  })
);
...
router.put("/update_user",controller.isAuthenticated, async (req, res) => {
  try {
...
router.get("/get_user_by_mail", async (req, res) => {
  try {
    ...
    if (typeof mail !== "string") {
      return res.status(400).json({
        success: false,
        message: "Le type du mail de la catégorie est incorrect.",
      });
    }
    ...
    if (!mail) {
      return res.status(400).json({
        success: false,
        message:
          "Le mail de la catégorie est requis pour retourner les valeurs de Categorie.",
      });
    }
...
// Utilisation du middleware pour la route protégée
router.get("/protected", controller.isAuthenticated, (req, res) => {
  res.json({
    success: true,
    message: "Vous êtes connecté.",
    mail: req.session.mail,
